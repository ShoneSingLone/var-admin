  ...
  function FLIP() {
    ...
    useEffect(() => {
      if (showModal) {
        ...
        const translateX = firstState.width - lastState.width;
        const translateY = firstState.height - lastState.height;
    ...
    function Modal(props) {
      return createPortal(<div className="modal-container" onClick={props.onClick}>{props.children}</div>, bodyContainer);
    }
      ...
      if (showModal) {
        return (
          <Modal onClick={() => setShowModal(false)}>
            <div style={modalItemStyle} className="pic-item modal-item" ref={previewRef} >
              <pre>{JSON.stringify(firstState, null, 1)}</pre>
              <pre>{JSON.stringify(lastState, null, 1)}</pre>
            </div>
          </Modal>
        );
      } else {
