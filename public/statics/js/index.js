(function () {
  var vm = window.vm = new Vue({
    ...
    methods: {
      initIndexedDbInfo: function initIndexedDbInfo() {
        var self = this;
        // self.checkDict()
        ...
        return Promise.all([
          ...
          self.$http.get('/sys/dict/loadStatu')
            ...
            .then(function () {
              /* 修改版本号 */
              window.idb.set('sysDictVersion', newDBversion);
              return window.Vue.initAppStaticResource()
                  .then()
                  .catch(console.error.bind(console));
            });
        /*
          sysDict .find({ dictType: { $eq: 'GENDER' }, pid: { $ne: '0' } }) .then(function (_) { console.log('"GENDER"', JSON.stringify(_, null, 2)); });
          sysDict .find({ dictType: { $eq: 'AREACODE' }, pid: { $ne: '0' } }) .then(function (_) { console.log('AREACODE', JSON.stringify(_, null, 2)); });
          sysDict .find({ dictType: { $eq: 'IDCARDTYPE' }, pid: { $ne: '0' } }) .then(function (_) { console.log('IDCARDTYPE', JSON.stringify(_, null, 2)); });
          sysDict .find({ dictType: { $eq: 'CAPITALCURRENCYTYPE' }, pid: { $ne: '0' } }) .then(function (_) { console.log('CAPITALCURRENCYTYPE', JSON.stringify(_, null, 2)); });
         */
      },
      // 设置导航条, 布局风格
            ...
            <el-menu-item index="1">\
              <el-dropdown placement="bottom" :show-timeout="0">\
                ...
                <el-dropdown-menu slot="dropdown">\
                  <el-dropdown-item v-for="(val, key) in $i18n.messages" :key="key" @click.native="$i18n.locale = key">{{ val._lang }}</el-dropdown-item>\
                </el-dropdown-menu>\
              </el-dropdown>\
            </el-menu-item>\
            <el-menu-item index="2">\
              <a href="//www.xinyegk.com/" target="_blank">\
      ...
      <main class="aui-content aui-content--tabs">\
        ...
        <el-tabs v-model="selfContentTabsActiveName" @tab-click="tabSelectedHandle" @tab-remove="tabRemoveHandle">\
          <el-tab-pane\
            ...
            <iframe v-show="iframeVisibleHandle(item)" :src="item.url" width="100%" height="100%" frameborder="0" scrolling="yes"></iframe>\
          </el-tab-pane>\
        </el-tabs>\
      </main>\
    ',
